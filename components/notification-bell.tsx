  ...
  return (
    <div className="relative">
      <button
        ...
        className="relative w-9 h-9 rounded-xl bg-muted hover:bg-accent flex items-center justify-center transition-colors"
        aria-label={t('title')}
      >
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-11 z-50 w-80 max-h-[480px] overflow-y-auto bg-card rounded-2xl border border-border shadow-xl">
              ...
              <div className="divide-y divide-border">
                {notifications.map((n) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 hover:bg-accent transition-colors ${!n.read ? 'bg-blue-50/50 dark:bg-blue-950/20' : ''}`}
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className={`flex-1 min-w-0 ${n.read ? 'pl-5' : ''}`}>
                        ...
                        <p className="text-[11px] text-muted-foreground mt-1">{formatRelativeTime(n.created_at, locale)}</p>
                      </div>
                    </div>
                  </button>
                ))}
